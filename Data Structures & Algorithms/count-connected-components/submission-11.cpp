...
class Solution
{
    ...
    void unionFind(int node_1, int node_2, std::vector<int> &ranks, std::vector<int> &parents)
    {
  ...
  public:
    int countComponents(int n, std::vector<std::vector<int>> &edges)
    {
        ...
        // making sure all parents point to the root
        for (int i{}; i < n; i++)
        {
            findParent(i, parents);
        }

        return std::unordered_set<int>(parents.begin(), parents.end()).size();
    }
};
