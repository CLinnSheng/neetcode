...
// Intuition: DFS and backtracking and also checking whether is it a palindrome at the same time
...
public:
    ...
    void dfs_backtracking(std::vector<std::vector<string>> &ans, std::vector<string> &subset, int index, const std::string &str) {
        if (index == str.length()) {
